...
def dateText(delta, tenderPeriod, periodSemster, periodOther):
    ...
    if delays <= 0:
        ...
        line3 = "-" + '\n'+"(تمديدات اخرى ({three}) + تمديد الصيف والشتاء ({four}) )".format(
            three=periodOther, four=periodSemster)
        #line4=" =   ({five})".format(five=delays)
        explanation = line1+line2+line3
        delays_2 = num2words(delays, lang='ar') + ' ' + \
            yum + " " + "( " + str(delays) + " )"
        value = {1: explanation, 2: delays}
